...
//创建yuv 纹理
    ...
    m_yTexture_data = std::shared_ptr<uint8_t>(new uint8_t[width * height]);
    m_uTexture_data = std::shared_ptr<uint8_t>(new uint8_t[width * height/4]);
    m_vTexture_data = std::shared_ptr<uint8_t>(new uint8_t[width * height/4]);

    //copy y
    const uint8_t* p_ydata = Buffer[0];
    uint8_t* ypoint =(uint8_t*) m_yTexture_data.get();
    ...
    uint8_t* upoint = (uint8_t*)m_uTexture_data.get();
    ...
    uint8_t* vpoint =(uint8_t*) m_vTexture_data.get();
	for (int i = 0; i < height/2; i++)
	{
